...
var pdk_NetManager = app.BaseClass.extend({
    ...
    Init:function(){
        ...
        // 添加网络日志开关
        this.enableNetLog = true;
        this.enableDetailedLog = true;
    },

    // 简单的格式化网络日志输出
    FormatNetLog: function(type, data, additionalInfo = "") {
        if (!this.enableNetLog) return;
        ...
        if (this.enableDetailedLog) {
            if (typeof data === 'object') {
                console.log(`${logPrefix} ${additionalInfo}`);
                console.log("数据内容:", JSON.stringify(data, null, 2));
                console.log("数据类型:", typeof data);
                console.log("数据大小:", JSON.stringify(data).length, "字符");
                if (data.Head !== undefined) {
                    console.log("封包头:", data.Head);
                }
                if (data.routeName !== undefined) {
                    console.log("路由名称:", data.routeName);
                }
            } else {
                console.log(`${logPrefix} ${additionalInfo}:`, data);
            }
        } else {
            console.log(`${logPrefix} ${additionalInfo}:`, data);
        }
    ...
    // 格式化发送数据日志
    FormatSendLog: function(eventName, sendPack, serverType = "GameServer") {
        this.FormatNetLog("SEND", sendPack, `发送到${serverType} - 事件: ${eventName}`);
    },

    // 格式化接收数据日志
    FormatReceiveLog: function(routeName, receivePack, serverType = "GameServer") {
        this.FormatNetLog("RECV", receivePack, `从${serverType}接收 - 路由: ${routeName}`);
    },

    // 格式化HTTP发送日志
    FormatHttpSendLog: function(sendPack, serverUrl) {
        this.FormatNetLog("HTTP_SEND", sendPack, `HTTP发送到: ${serverUrl}`);
    },

    // 格式化HTTP接收日志
    FormatHttpReceiveLog: function(receivePack, serverUrl) {
        this.FormatNetLog("HTTP_RECV", receivePack, `HTTP从${serverUrl}接收`);
    },
	...
	SendPack:function(eventName, sendPack, callback=null, errorCallback=null){
		// 格式化打印发送数据
		this.FormatSendLog(eventName, sendPack, "GameServer");
    ...
    OnReceive:function(routeName, receivePack){
        // 格式化打印接收数据
        this.FormatReceiveLog(routeName, receivePack, "GameServer");
    ...
    SendHttpPack:function(sendPack){
        // 格式化打印HTTP发送数据
        this.FormatHttpSendLog(sendPack, "HTTP");
...
*/
	...
	OnReceiveHttpPack:function(serverUrl, httpResText){
		...
		// 格式化打印HTTP接收数据
		this.FormatHttpReceiveLog(receivePack, serverUrl);
