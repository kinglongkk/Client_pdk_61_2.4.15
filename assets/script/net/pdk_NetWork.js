...
var pdk_NetWork = app.BaseClass.extend({
	...
	// 格式化WebSocket发送数据日志
	FormatWebSocketSendLog: function(head, message, strMsg) {
		if (!this.IsDevelopment()) return;
		
		const timestamp = new Date().toISOString();
		console.log(`[${timestamp}] [WS_SEND] WebSocket发送数据`);
		console.log("消息类型:", head.messageType);
		console.log("事件名称:", head.event);
		console.log("序列号:", head.sequence);
		console.log("消息内容:", JSON.stringify(message, null, 2));
		console.log("消息字符串:", strMsg);
		console.log("消息长度:", strMsg.length, "字符");
	},

	// 格式化WebSocket接收数据日志
	FormatWebSocketReceiveLog: function(body, errorCode, eventName, sequence) {
		if (!this.IsDevelopment()) return;
		
		const timestamp = new Date().toISOString();
		console.log(`[${timestamp}] [WS_RECV] WebSocket接收数据`);
		console.log("事件名称:", eventName);
		console.log("序列号:", sequence);
		console.log("错误码:", errorCode);
		console.log("消息内容:", JSON.stringify(body, null, 2));
		console.log("消息类型:", typeof body);
		if (typeof body === 'object') {
			console.log("消息大小:", JSON.stringify(body).length, "字符");
		}
	},
	...
	OnMessage:function(event){
		...
		try{
			...
			// 格式化打印WebSocket接收数据
			this.FormatWebSocketReceiveLog(body, errorCode, eventName, sequence);
	...
	write:function(head, message) {
		...
		// 格式化打印WebSocket发送数据
		this.FormatWebSocketSendLog(head, message, strMsg);
...
exports.GetModel = function(){
    if(!g_pdk_NetWork){
	    g_pdk_NetWork = new pdk_NetWork();
    }
    return g_pdk_NetWork;
}