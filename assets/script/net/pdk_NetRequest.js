...
var pdk_NetRequest = app.BaseClass.extend({
	...
	// 格式化HTTP请求日志
	FormatHttpRequestLog: function(serverUrl, argString, requestType, sendPack, dataStr) {
		// 简单的HTTP请求日志
		const timestamp = new Date().toISOString();
		console.log(`[${timestamp}] [HTTP_REQUEST] HTTP请求详情`);
		console.log("请求URL:", serverUrl);
		console.log("请求参数:", argString);
		console.log("请求类型:", requestType);
		console.log("请求数据:", JSON.stringify(sendPack, null, 2));
		console.log("请求数据字符串:", dataStr);
		console.log("请求数据大小:", dataStr.length, "字符");
		console.log("完整URL:", [serverUrl, argString].join(""));
	},
	...
	SendHttpRequest:function(serverUrl, argString, requestType, sendPack){
		...
		// 格式化打印HTTP请求详情
		this.FormatHttpRequestLog(serverUrl, argString, requestType, sendPack, dataStr);
