...
cc.Class({
    ...
    ShowAddDouble: function(dataPos, addDouble) {
        // let uiPos = this.RoomPosMgr.GetUIPosByDataPos(dataPos);
        // let path = "sp_seat0" + uiPos + "/head" + "/SYUIPublicHead" + uiPos + "/otherNode";
    ...
    HideAllBsNode: function() {
        ...
        //     let path = "sp_seat0" + uiPos + "/head" + "/SYUIPublicHead" + uiPos;
    ...
    AddHead: function() {
        ...
        for (let idx = 0; idx < posList.length; idx++) {
            ...
            let node = this.GetWndNode(path);
            if (!node.getChildByName("SYUIPublicHead" + uiPos)) {
                ...
                let headScript = head.getComponent(app.subGameName + "_SYUIPublicHead");
                headScript.Init(uiPos, posList[idx].pos, cc.v2(0, 0), this.Left[uiPos]);
            } else {
                let headNode = node.getChildByName("SYUIPublicHead" + uiPos);
                // headNode.getChildByName("otherNode").active = false;
                // headNode.getChildByName("otherNode").getChildByName("bsPrefab").active = false;
                let headScript = headNode.getComponent(app.subGameName + "_SYUIPublicHead");
                headScript.Init(uiPos, posList[idx].pos, cc.v2(0, 0), this.Left[uiPos]);
            }
        }
    },
    ...
    ClearHead: function() {
        for (let idx = 0; idx < this.Define.MaxPlayer; idx++) {
            let path = "sp_seat0" + idx + "/head" + "/SYUIPublicHead" + idx;
            let node = this.GetWndNode(path);
            if (node) {
                let headScript = node.getComponent(app.subGameName + "_SYUIPublicHead");
                headScript.OnClose();
            }
        }
    },
    ...
    OnShow: function() {
        ...
        this.HideAll();
        this.unscheduleAllCallbacks();
        // this.LocalDataManager.SetConfigProperty("SysSetting","GameBackMusic","back_1");
