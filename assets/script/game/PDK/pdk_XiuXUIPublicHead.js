...
cc.Class({
    ...
    UpDateLabSportsPoint:function () {
        ...
        if(this.playerInfo && typeof(this.playerInfo['sportsPoint'])!="undefined"){
            ...
            let playerLength = 0;
            allPlayerInfo=room.GetRoomPosMgr().GetRoomAllPlayerInfo();
            playerLength = Object.keys(allPlayerInfo).length;

            let roomPosMgr = room.GetRoomPosMgr();
            carryingPoints = roomPosMgr.GetCarryingPoints();
            console.log("carryingPoints",carryingPoints);
            // carryingPoints = this.playerInfo.carryingPoints;
            if (playerLength == 2) {
                    sportsPoint = carryingPoints;
                    let cyp = parseFloat(carryingPoints).toFixed(2);
                    let addCpy = parseFloat(this.playerInfo.realPoint).toFixed(2);
                    sportsPoint = parseFloat(cyp) + parseFloat(addCpy);
                    if (sportsPoint > carryingPoints) sportsPoint = carryingPoints; 
            } else {
                let cyp = parseFloat(carryingPoints).toFixed(2);
                let addCpy = parseFloat(this.playerInfo.realPoint).toFixed(2);
                sportsPoint = parseFloat(cyp) + parseFloat(addCpy);
            ...
            console.log("sportsPoint",sportsPoint);
