...
var PDKRoomPosMgr = app.BaseClass.extend({
    ...
    OnReload: function () {
        ...
        this.facePos = -1;
        this.posCount = -1;
        this.carryingPoints = 0;
    },


    //-----------------------回调函数-----------------------------
    OnInitRoomPosData: function (roomPosInfoList, carryingPoints) {
        this.dataInfo = {};
        ...
        for (let index = 0; index < this.posCount; index++) {
            ...
            roomPosInfo["carryingPoints"] = carryingPoints;
    ...
    GetCarryingPoints: function () {
        return this.carryingPoints;
    },
    ...
    OnSetEnd: function (setEnd) {
        ...
        for (let pos in this.dataInfo) {
            ...
            //比赛分
            if (typeof (playerInfo.sportsPoint) != "undefined") {
                let realPoint = Math.abs(parseFloat(playerInfo["realPoint"]).toFixed(2));
                let addRealPoint = Math.abs(parseFloat(this.carryingPoints).toFixed(2));
                let totalRealPoint = realPoint - addRealPoint;
                playerInfo["totalRealPoint"] = totalRealPoint;
                this.dataInfo[pos]['totalRealPoint'] = totalRealPoint;
            }
        }
    },
