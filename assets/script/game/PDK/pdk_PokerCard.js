...
var PokerCard = app.BaseClass.extend({
    ...
    LoadAllPokerRes:function(){
        ...
        if(0 != this.pokerNewList.length)
            return;
        cc.resources.loadDir("texture/new_poker",cc.SpriteFrame, function(err, assets){
            if(err){
                cc.error(err);
                return;
            }
        ...
        if(0 != this.pokerLpList.length)
            return;
        cc.resources.loadDir("texture/lp_poker",cc.SpriteFrame, function(err, assets){
            if(err){
                cc.error(err);
                return;
            }
        ...
        if(0 != this.pokerSYList.length)
            return;
        cc.resources.loadDir("texture/sy_poker",cc.SpriteFrame, function(err, assets){
            if(err){
                cc.error(err);
                return;
            }
        ...
        if(0 != this.pokerXiuXList.length)
            return;
        cc.resources.loadDir("texture/xiux_poker",cc.SpriteFrame, function(err, assets){
            if(err){
                cc.error(err);
                return;
            }
